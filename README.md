...
Static site. No build tools, no dependencies. Open `docs/index.html` in a browser, or run the
local server below.
...
| File | What's on it |
| --- | --- |
| `docs/index.html` | Home — hero, the five service verticals, why one contact matters |
| `docs/stays.html` | Homestays and service apartments |
| `docs/celebrations.html` | Events hosted at home |
| `docs/journeys.html` | Temple journeys, rides, Hidden Mangalore |
| `docs/about.html` | The problem, how you work, the four-step journey, vision |
| `docs/contact.html` | Enquiry form and contact details |

## Where to add your details

**Copy and page structure** — either edit the HTML in `docs/` directly, or edit `build_site.py`
...
tour. Two encodes live in `docs/assets/video/`: `hero.mp4` (a wide band, for desktop) and
`hero-portrait.mp4` (the full tall frame, for phones); the script picks one at load.
`docs/assets/img/hero-poster.jpg` shows until playback starts. The folder's README explains the
...
`docs/assets/img/pages/`: `stays.jpg`, `celebrations.jpg`, `journeys.jpg`, `about.jpg`.
...
band from `docs/assets/img/cards/`: `stays.jpg`, `celebrations.jpg`, `rides.jpg`, `temples.jpg`,
...
**Photographs** — drop yours into `docs/assets/img/` and swap the `src` in the HTML. Two
...
`EMAIL`. Change them there and rebuild, or find-and-replace across `docs/*.html`.

**Colours and type** are CSS variables at the top of `docs/assets/css/styles.css`. The sage
`#5C745C` is sampled from your logo; the terracotta `#B4553A` is a Mangalore roof-tile red used
sparingly as the accent.

## The enquiry form

`docs/assets/js/main.js` currently opens WhatsApp with the enquiry pre-filled, because there is
